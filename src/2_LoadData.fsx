...
let phrases : Phrases list =
    let json = PsychoPhrases.Load("../data/phrases.json")
    json |> Array.map (fun r -> 
        {
            InputPattern = {
                            Contents = r.Pattern |> cleanText |> parseText
                            IsQuestion = isQuestion r.Pattern
            }
            AnswerPatterns = 
                r.Answers |> Array.map(parseText)
        })
    |> Array.toList
...
type Reflect = JsonProvider<"../data/reflections.json">
let refls = Reflect.GetSamples() |> Array.map(fun i -> i.Input, i.Output) |> dict

// 2. Reflect all the words in the text that can be reflected
// based on the dictionary
let reflect (text : string list) = 
    text |> List.map (fun w -> if refls.ContainsKey w then refls.[w] else w)
