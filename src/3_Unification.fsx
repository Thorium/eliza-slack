...
let rec unification pattern input acc =
  ...
  | Wildcard::t, [] -> unification t input acc
  //     * match all of the pattern (with wildcard)
  //       with the rest of the input (assign the first word to the wildcard)
  | Wildcard::t, h::t2 -> 
        match unification pattern t2 (List.concat[acc;[h]]) with
        | Some x -> Some x
        | None -> unification t input (List.concat[acc])
  //  - if the pattern & input start with the same word, skip them
  | Word(x)::t1, Word(y)::t2 when x = y -> unification t1 t2 acc
